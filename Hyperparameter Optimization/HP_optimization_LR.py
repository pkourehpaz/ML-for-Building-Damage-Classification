...
stdc = StandardScaler()
stdc.fit_transform(X_f)
X_n = stdc.transform(X_f)
...
c_values = [0.001, 0.01, 0.1, 0.5, 1, 2, 5, 6, 8, 10]
...
grid_result = grid_search.fit(X_n, Y)
# summarize results
...
p1=plt.errorbar((c_values), means_train, color='k', yerr=asymmetric_error_train, linewidth=1.0, capsize=1, capthick=1.5)
...
p2=plt.errorbar((c_values), means_test, color=(0.78,0,0), yerr=asymmetric_error_test, linestyle='dashed', linewidth=1.0, capsize=1, capthick=1.5)
